...
**Step 0: Verify and update imports at the top of `src/chain/router.ts`:**
- Ensure `ChatCompletionRequest` is imported from `'../shared/types.js'`. Add it to the existing import if not present: `import type { ChatCompletionRequest } from '../shared/types.js';`
- Verify `AllProvidersExhaustedError` is imported from `'../shared/errors.js'` (should exist from Phase 1, but confirm before proceeding). If missing, add: `import { AllProvidersExhaustedError } from '../shared/errors.js';`
- Update the chain types import to include `StreamChainResult`: `import type { Chain, ChainResult, StreamChainResult } from './types.js';`
...
Note the `signal` parameter: it is forwarded to `adapter.chatCompletionStream()`. If the client disconnects during the pre-stream waterfall phase, the AbortError propagates up and the route handler catches it cleanly.
  ...
  <done>executeStreamChain exists in src/chain/router.ts. StreamChainResult type exists in src/chain/types.ts. Pre-stream waterfall skips exhausted providers, handles 429 with fallthrough, propagates AbortError, throws AllProvidersExhaustedError when all fail. All imports (ChatCompletionRequest, AllProvidersExhaustedError, StreamChainResult) are explicitly verified/added.</done>
</task>
...
1. Add/update imports at the top of `src/api/routes/chat.ts`:
```typescript
...
```
Note: Some of these imports may already exist (e.g., AllProvidersExhaustedError from Phase 1). Do not duplicate -- merge into existing import statements.
...
3. The streaming branch implementation. IMPORTANT: Use a definite assignment assertion (`let streamResult!: StreamChainResult;`) to avoid TypeScript "used before assigned" errors. The catch block always exits via `return` or `throw`, so `streamResult` IS guaranteed to be assigned after the try-catch, but TypeScript's control flow analysis cannot prove this. The `!` assertion tells TypeScript we guarantee assignment.
...
Also add the `StreamChainResult` import at the top of the file:
```typescript
import type { StreamChainResult } from '../../chain/types.js';
```
...
- TypeScript compiles cleanly (definite assignment assertion on streamResult)
  </done>
</task>
